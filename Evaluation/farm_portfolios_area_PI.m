function [improvement, risk] = farm_portfolios_area_PI(data, predicted_yield,...
    covariance_matrix, real_yield, VOT, YDT)
        ...
        for fm=1:number_of_farms_year
            ...
            if (sum(mask) > 1)
                [varieties, percentage] = optimise_portfolio_PI(farm_yields', ...
                    covariance_matrix, PI(fm), mask, 'dont-plot'); %
            elseif (sum(mask) == 1)
                [~,varieties] = max(mask);
                percentage = 1;
            else
